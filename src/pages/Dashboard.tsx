...
import { Lock, Unlock, Calendar, Star, Sparkles, Eye, Share2, Settings, Users, MessageCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [activeView, setActiveView] = useState<'capsules' | 'friends' | 'chat'>('capsules');
  ...
  const handleStartChat = (friendId: string, friendName: string) => {
    setChatFriend({ id: friendId, name: friendName });
    setActiveView('chat');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">Loading your capsules...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            <h1 className="font-serif text-3xl sm:text-4xl font-bold text-slate-800 dark:text-slate-200 mb-2">
              Welcome back, {user?.user_metadata?.full_name || 'Time Traveler'}!
            </h1>
            <p className="text-lg text-slate-600 dark:text-slate-400">
              Manage your time capsules and connect with friends
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={signOut} className="border-lavender-200 dark:border-slate-600">
        ...
        <div className="flex justify-center gap-2 mb-8">
          <Button
            ...
            className={activeView === 'capsules' ? 'bg-lavender-500 hover:bg-lavender-600' : 'border-lavender-200 dark:border-slate-600'}
          >
          ...
          <Button
            ...
            className={activeView === 'friends' ? 'bg-lavender-500 hover:bg-lavender-600' : 'border-lavender-200 dark:border-slate-600'}
          >
        ...
        {/* Stats Cards - Only show for capsules view */}
        {activeView === 'capsules' && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="border-lavender-200 dark:border-slate-600">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm text-slate-600 dark:text-slate-400">Still Locked</div>
              </CardContent>
            </Card>
            <Card className="border-lavender-200 dark:border-slate-600">
              <CardContent className="p-4 text-center">
          ...
          <Tabs defaultValue="capsules" className="space-y-6">
            ...
            <TabsContent value="capsules" className="space-y-6">
              ...
              <div className="flex justify-center gap-2">
                <Button
                  ...
                  className={filter === "all" ? "bg-lavender-500 hover:bg-lavender-600" : "border-lavender-200 dark:border-slate-600"}
                >
                  All Capsules
                </Button>
                <Button
                  ...
                  className={filter === "locked" ? "bg-lavender-500 hover:bg-lavender-600" : "border-lavender-200 dark:border-slate-600"}
                >
                  <Lock className="w-4 h-4 mr-2" />
                  Locked
                </Button>
                <Button
                  ...
                  className={filter === "unlocked" ? "bg-lavender-500 hover:bg-lavender-600" : "border-lavender-200 dark:border-slate-600"}
                >
              ...
              {filteredCapsules.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <h3 className="font-serif text-2xl font-semibold text-slate-700 dark:text-slate-300 mb-2">No capsules found</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredCapsules.map((capsule) => {
                    ...
                    return (
                      <Card key={capsule.id} className="border-lavender-200 dark:border-slate-600 hover:shadow-lg transition-all duration-300">
                        <CardHeader className="pb-3">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-1">
                              ...
                              {capsule.is_private && (
                                <Badge variant="outline" className="border-slate-300 dark:border-slate-600">Private</Badge>
                              )}
                            </div>
                          </div>
                        </CardHeader>
                            ...
                            {unlocked ? (
                              <div className="space-y-2">
                                {capsule.content_type === "text" && (
                                  <p className="text-slate-700 dark:text-slate-300 line-clamp-3">{capsule.content}</p>
                                )}
                                ...
                                {capsule.content_type === "drawing" && (
                                  <div className="w-full h-32 bg-gradient-to-br from-lavender-50 to-peach-50 dark:from-slate-700 dark:to-slate-600 rounded-lg flex items-center justify-center">
                                    <Star className="w-8 h-8 text-lavender-400" />
                                  </div>
                                )}
                              </div>
                            ) : (
                              <div className="h-32 bg-gradient-to-br from-lavender-100 to-skyblue-100 dark:from-slate-700 dark:to-slate-600 rounded-lg flex items-center justify-center">
                                <div className="text-center">
                                  <Lock className="w-8 h-8 text-lavender-400 mx-auto mb-2" />
                                  <p className="text-sm text-slate-600 dark:text-slate-400">Locked until unlock date</p>
                                </div>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            {unlocked && (
                              <Button 
                                ...
                                onClick={() => handleShareCapsule(capsule.id)}
                              >
            ...
            <TabsContent value="shared">
              <div className="text-center py-16">
                ...
                <h3 className="font-serif text-2xl font-semibold text-slate-700 dark:text-slate-300 mb-2">Shared Capsules</h3>
                <p className="text-slate-500 dark:text-slate-400">Capsules shared with you will appear here</p>
              </div>
            </TabsContent>
          </Tabs>
        )}

        {activeView === 'friends' && !chatFriend && (
          <Friends onStartChat={handleStartChat} />
        )}

        {activeView === 'chat' && chatFriend && (
          <Chat 
